...
class PagesController extends Controller
{
    public function index(Request $request)
    {
        ...
        $active_user_count = 0;
        $daily_detail_view_count = 0;
        $daily_score_view_count = 0;
        $daily_active_user_count = 0;
        $daily_personal_view_count = 0;
        ...
        $dailyDetailViewCounts = [];
        $dailyScoreViewCounts = [];
        $dailyActiveUserCounts = [];
        $dailyPersonalViewCounts = [];
        $statDates = [];
        $statMonths = [];
        $statDailyDates = [];
        $statDailyMonths = [];
        ...
        $initalDate = $today->copy()->subMonth()->addDay(); // Resta un mes y agrega un día para incluir hoy        
        // Calcular la diferencia en días
        $numDays = $today->diffInDays($initalDate) + 1;
        ...
        for ($i = 0; $i < $numDays; $i++) {
            ...
            if ($startDate->dayOfWeek == 1 && ($i - $lastPrint) > 2 && ($lastI - $i) > 2) {
                $lastPrint = $i;
            ...
            $detailViewsCount = DB::table('log_view_music_score_details')
                ->whereBetween('created_at', [$startDate, $endDate])
                ->count();
            $dailyDetailViewCounts[] = $detailViewsCount;
            $daily_detail_view_count += $detailViewsCount;
            ...
            $personalViewsCount = DB::table('log_display_personal_scores')
                ->whereBetween('created_at', [$startDate, $endDate])
                ->count();
            $dailyPersonalViewCounts[] = $personalViewsCount;
            $daily_personal_view_count += $personalViewsCount;
        ...
        if ($request->ajax()) {
            ...
            return view('pages.dashboard')
                ...
                ->with('request_count', round($request_count / $numWeeks, 0))
                ->with('daily_detail_view_count', round($daily_detail_view_count / $numDays, 0))
                ...
                ->with('daily_personal_view_count', round($daily_personal_view_count / $numDays, 0))
                ...
                ->with('dailyDetailViewCounts', $dailyDetailViewCounts)
                ->with('dailyScoreViewCounts', $dailyScoreViewCounts)
                ->with('dailyActiveUserCounts', $dailyActiveUserCounts)
                ->with('dailyPersonalViewCounts', $dailyPersonalViewCounts)
