...
class UserController extends Controller
{
	...
	public function update(Request $request)
	{
		...
		$request->validate([
			...
			'premium_trial_count' => ['nullable', 'numeric'],
		]);
		...
		if ($request->sync_subscr_type != null && $request->sync_subscr_type >= 0) {
			...
			// $subscriptionService->checkPlanOffer($id, $planType);
			$subscriptionService->updateSubscriptionById($id, $planType);
		}
		//
		// Actualizar o crear el registro de PremiumTrial
		if ($request->filled(['premium_trial_count']) && isset($request->premium_trial_count)) {
			$user->premiumTrial()->updateOrCreate(
				[], // Si no hay condición, actualizará el único relacionado por hasOne
				[
					'used_count' => $request->premium_trial_count,
				]
			);
		}
		//
	...
	public function sendUserRegisteredEmail($id)
	{
		$user = User::findOrFail($id);
	...
	public function sendPlanOfferEmail($id)
	{
		$user = User::findOrFail($id);
		Mail::to($user->email)->send(new PlanOfferEmail($user));
		return redirect()->back()->with('status', 'email-sent');
	}
}
