            ...
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="">
                    ...
                    <form method="post" action="{{ route('user.update', $user->id) }}" class="mt-6 space-y-6">
                        ...
                        <div class="alert alert-warning" role="alert" id="subscription-warning" style="display: none;">
                            <strong>Warning!</strong> Changing the subscription type may affect the user's current plan and result in data loss.
                        </div>
                        ...
                        <div>
                            ...
                            @if(isset($phoneNumber))
                                ...
                                <input type="tel" id="phone" name="telephone" class="intl-tel-input" value="{{ old('telephone', $phoneNumber) }}" {!! isset($countryCode) ? 'data-country="' . $countryCode . '"' : '' !!}>
                            @else
                                <input type="hidden" name="country_code" id="country_code" value="">                                  
                        ...
                        <div class="user_toggle">
                            <x-input-label for="status" :value="__('Status')" />
                                ...
                                <input data-id="{{$user->id}}" name="user_status" class="toggle-class" type="checkbox" data-on="Active" data-off="Suspended"  data-toggle="toggle" data-offstyle="danger" {{ $checked }} >
                            <x-input-error class="mt-2" :messages="$errors->get('status')" />
                        </div>
                        <?php $level = $subscription_data['level']; ?>
                        <div>
                            <x-input-label for="sync_subscr_type" :value="__('Change subscription plan (warning!)')" />
                            <select id="sync_subscr_type" name="sync_subscr_type" class="mt-1 block w-full" style="background-color: lightgrey" required>
