...
<div class="login-signup text-dark">
  <div class="mb-20">
    <h3>Sign Up</h3>
    <p class="opacity-60">Enter your details to create your account</p>
  </div>
  <form class="text-center form" id="kt_login_signup_form" method="POST" action="{{route('register_user')}}">
    @csrf
    ...
    <div class="form-group">
      <button id="kt_login_signup_submit" type="submit"
        class="py-3 m-2 btn btn-pill btn-outline-dark font-weight-bold opacity-90 px-15">Sign
        Up</button>
      <a id="kt_login_signup_cancel"
        href="{{ route('welcome') }}" class="py-3 m-2 btn btn-pill btn-outline-dark font-weight-bold opacity-70 px-15">Cancel</a>
    </div>
  </form>
</div>
