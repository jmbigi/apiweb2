  ...
  <form class="form" id="kt_login_signin_form2" method="POST" action="{{ route('login') }}">
    ...
    <div class="flex-wrap px-8 form-group d-flex justify-content-between align-items-center">
      <div class="checkbox-inline">
        <label class="m-0 text-dark checkbox checkbox-outline checkbox-dark">
          <input type="checkbox" name="remember" />
          <span></span>Remember me</label>
      </div>
      <a href="{{ route('password.request') }}" id="kt_login_forgot" class="text-dark font-weight-bold">
        {{ __('Forgot password ?') }}
      </a>
    </div>
    <div class="mt-10 text-center form-group">
      <button id="kt_login_signin_submit2" type="submit"
        class="py-3 btn btn-pill btn-outline-dark font-weight-bold opacity-90 px-15">Sign
        In</button>
    </div>
  </form>
  <!-- <div class="mt-10">
    <span class="mr-4 opacity-70">Don't have an account yet?</span>
    <a href="" id="kt_login_signup" class="text-dark font-weight-bold">Sign Up</a>
  </div> -->
</div>
